...
export class OperisOnboarding extends LitElement {
  ...
  disconnectedCallback() {
    ...
    window.removeEventListener("scroll", () => this.updateTargetPosition(), true);
  }
  ...
  private waitForTargetAndUpdate(retries = 15) {
    ...
    if (isVisible) {
      ...
      console.warn(`[onboarding] Target not found for step: ${step.id}, selector: ${step.target}`);
      if (this.currentStep < this.steps.length - 1) {
        this.currentStep++;
        this.waitForTargetAndUpdate();
      } else {
  ...
  render() {
              ...
              <div class="step-dots">
                ${this.steps.map(
                  (_, i) => html`
                    <div
                      class="step-dot ${
                        i === this.currentStep ? "active" : i < this.currentStep ? "completed" : ""
                      }"
                    ></div>
                  `,
                )}
    ...
    return html`
      ...
      <!-- Tooltip Card -->
      <div
        class="tooltip-card ${this.targetRect && !this.isAnimating ? "visible" : ""}"
        data-side="${this.tooltipSide}"
        style="top: ${this.tooltipPosition.y}px; left: ${this.tooltipPosition.x}px;"
      >
      ...
      <!-- Animated Cursor -->
      <div
        class="onboarding-cursor ${this.showCursor ? "visible" : ""} ${
          this.cursorClicking ? "clicking" : ""
        }"
        style="top: ${this.cursorPosition.y}px; left: ${this.cursorPosition.x}px;"
      >
