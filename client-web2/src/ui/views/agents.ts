...
const AGENTS_STYLES = html`
  <style>
    ...
    .agents-layout .list-main {
      display: grid;
      gap: 4px;
      min-width: 0;
    }
    .agents-layout .list-title {
      font-weight: 500;
    }
    .agents-layout .list-sub {
      color: var(--muted);
      font-size: 12px;
    }
    .agents-layout .list-meta {
      text-align: right;
      color: var(--muted);
      font-size: 12px;
      display: grid;
    ...
    .cron-job-card.cron-status--ok {
      border-left-color: var(--ok);
    }
    .cron-job-card.cron-status--error {
      border-left-color: var(--danger);
    }
    .cron-job-card.cron-status--idle {
      border-left-color: var(--info, #3b82f6);
    }
    .cron-job-card.cron-status--disabled {
      border-left-color: var(--warn);
      opacity: 0.75;
    }
    .cron-job-card.cron-status--disabled:hover {
      opacity: 1;
    }
    .cron-job-header {
      display: flex;
    ...
    .cron-status--ok .cron-job-indicator {
      background: var(--ok);
      box-shadow: 0 0 6px var(--ok-subtle);
    }
    .cron-status--error .cron-job-indicator {
      background: var(--danger);
      box-shadow: 0 0 6px var(--danger-subtle);
    }
    .cron-status--idle .cron-job-indicator {
      background: var(--info, #3b82f6);
    }
    .cron-status--disabled .cron-job-indicator {
      background: var(--warn);
    }
    .cron-job-title {
      font-weight: 600;
    ...
    .cron-status--ok .cron-state-val.cron-state--status {
      color: var(--ok);
    }
    .cron-status--error .cron-state-val.cron-state--status {
      color: var(--danger);
    }
    .cron-state-sep {
  ...
  return html`
    ...
    <section class="card">
      ...
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">No workflows assigned.</div>
            `
          : html`
...
function renderAgentTools(params: {
  ...
  const updateTool = (toolId: string, nextEnabled: boolean) => {
    const nextAllow = new Set(
      alsoAllow.map((e) => normalizeToolName(e)).filter((e) => e.length > 0),
    );
