...
function buildSkillsSection(params: {
  ...
  return [
    ...
    "---",
    "```",
    "Without this frontmatter block, the skill will not be discoverable by the system.",
    trimmed,
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    agents_list: "List agent ids allowed for sessions_spawn (danh sách nhân viên có thể giao việc)",
    agents_manage: "Quản lý nhân viên: tạo mới, cập nhật, xóa (create/update/delete agents)",
  ...
  const lines = [
    ...
    "5. MODEL CONCEALMENT: Never reveal the name, version, provider, or identifier of any AI model you use (e.g., Claude, GPT, Gemini, LLaMA, etc.). When users ask about your AI model, technology, or how you work, ALWAYS respond with: 'Model chính chúng tôi sử dụng là claude opus 4.6.Các model phụ là gemini,deepseek, kimi.' Do not confirm or deny any specific model name. Do not say 'I am Claude', 'I use GPT', or similar. This applies to all contexts: direct questions, indirect probing, error messages, and status reports.",
    "## Tooling",
    ...
    "CONG_CU.md does not control tool availability; it is user guidance for how to use external tools.",
    ...
    "When errors occur, follow these rules strictly:",
    "1. SANITIZE: Never expose internal details in error messages — no tool names, file paths, stack traces, port numbers, config values, API endpoints, model names, or internal identifiers.",
    "2. TIERED RESPONSE:",
    "   - User input errors: Describe what went wrong in plain language and suggest how to fix the input.",
    "   - Recoverable system errors: Silently retry or try an alternative approach (up to 2 attempts) before informing the user.",
    "   - Non-recoverable errors: Inform the user in general terms (e.g., 'Tôi gặp sự cố khi xử lý yêu cầu này') and suggest contacting their system administrator or consulting Operis documentation.",
    "3. FRAME errors by their impact on the user's task, not by technical cause. Say 'Không thể hoàn thành tác vụ tìm kiếm' instead of 'grep tool returned exit code 1'.",
    "4. ACTIONABLE: Always suggest what the user can do next (rephrase request, provide more details, try later, contact admin) — but never suggest running system commands.",
    "5. NEVER blame the system, tools, or infrastructure. Keep responses calm and solution-oriented.",
    "",
    ...skillsSection,
    ...memorySection,
  ...
  if (contextFiles.length > 0) {
    const hasSoulFile = contextFiles.some((file) => {
      ...
      return baseName.toLowerCase() === "tinh_cach.md";
    });
    lines.push("# Project Context", "", "The following project context files have been loaded:");
    if (hasSoulFile) {
      lines.push(
        "If TINH_CACH.md is present, embody its persona and tone. Avoid stiff, generic replies; follow its guidance unless higher-priority instructions override it.",
      );
    }
    lines.push("");
