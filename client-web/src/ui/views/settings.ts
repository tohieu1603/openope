...
export function renderSettings(props: SettingsProps) {
      ...
      <div class="st-channel-card ${channel.connected ? 'connected' : ''}" style="--ch-color: ${brand.color}; --ch-bg: ${brand.bgColor}; --ch-gradient: ${brand.gradient};">
        ...
        <div class="st-channel-info">
          ...
          <span class="st-channel-status">
            ${channel.connected ? html`<span class="st-status-dot"></span>` : nothing}
            ${isZalo ? "Sắp có" : channel.connected ? "Đã kết nối" : "Chưa kết nối"}
          </span>
        </div>
        <div class="st-channel-action">
          ${isZalo ? html`
            <button class="st-btn-channel disabled" disabled>Sắp có</button>
  ...
  return html`
    ...
    <div class="st-page">
      ...
      ${loading ? html`<div class="st-loading"></div>` : html`
        <!-- Profile Hero -->
        <div class="st-hero">
