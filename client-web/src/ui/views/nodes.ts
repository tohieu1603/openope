...
import { clampText, formatAgo, formatList } from "../format-utils";
...
function resolveExecNodes(nodes: NodeInfo[]): BindingNode[] {
...
function resolveExecApprovalsNodes(nodes: NodeInfo[]): ExecApprovalsTargetNode[] {
...
export function renderNodes(props: NodesProps) {
  ...
  return html`
    <style>
      ...
      .chip-ok {
        background: var(--ok-subtle, #dcfce7);
        color: var(--ok, #16a34a);
      }
      .chip-warn {
        background: var(--warn-subtle, #fef3c7);
    ...
    ${renderDevices(props)} ${renderNodesList(props)}
  `;
}
          ...
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option
                    value="__default__"
                    ?selected=${askFallbackValue === "__default__"}
                  >
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">${role} · requested ${age}${repair}</div>
      </div>
      <div class="list-meta">
